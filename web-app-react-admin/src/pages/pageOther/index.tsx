...
import { apiPageOther } from '@/apis/index'
import { connect, useSelector } from 'react-redux'
import { IBaseParams, PageOtherVM } from '@/models';
import { checkPermission, SerializeParam } from '@/helpers/utils';
import { AlertDialogSlide, DivTable, useNotification } from '@/components';
import { commandId, functionId } from '@/constants/utilConstant';
import { tableHeadPageOther } from '@/models/tableHead';
...
function PageOther(props: IProps) {
    let history = useHistory();
    const dispatch = useNotification();
    const [data, setData] = useState<PageOtherVM[]>([]);
    ...
    useEffect(() => {
        props.setBreadcrumb([{ name: 'Danh sách các trang khác' }]);
    }, [])
    ...
    async function getData() {
        ...
        let newParam = { ...params };
        await apiPageOther.getPaging(serialParam).then((rsp) => {
            if (!rsp.isError) {
                setLoading(false)
    ...
    async function handleDelete() {
        await apiPageOther.delete(idSelect).then(async (rsp) => {
            if (!rsp.isError) {
    ...
    function renderHeader() {
        return <div className="pb-5 d-flex justify-content-end align-items-center ">
            {
                checkPermission(functionId.pageOther, commandId.create) && <button onClick={() => {
                    history.push(PATH.SETTING_PAGE_CREATE)
    ...
    function renderContent() {
        return <DivTable
            funcId={functionId.pageOther}
            data={data}
            header={tableHeadPageOther}
            onchangeParam={(e) => handleChangeParam(e)}
            pageSize={params.pageSize}
            page={params.page}
            total={params.totalCount}
            isLoading={isLoading}
            handleEdit={(id) => history.push(`${PATH.SETTING_PAGE_EDIT}${id}`)}
...
export default connect(mapStateToProps, mapDispatchToProps)(PageOther)
